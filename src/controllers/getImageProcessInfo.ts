...
const getImageProcessInfo = (req: Request, _: Response, next: NextFunction): void => {
  ...
  const ext = (req.query.ext as string) || (process.env.DEFAULT_IMAGE_FORMAT as string)
  const thumbImageName = `${imageName}-${width}x${height}.${ext}` as string
